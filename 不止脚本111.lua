...
infoSection:Paragraph({
    Title = "不止脚本",
    Desc = "作者关键词作者qq:477409523脚本还在测试中",
})
...
PlayerSection:Button({
    ...
    Callback = function()
        ...
        if not TargetPlayer or TargetPlayer == Player then
            WindUI:Notify({Title = "不止脚本", Content = "无玩家可甩飞", Duration = 2})
            return
        end
        ...
        local SkidFling = function(Target)
            ...
            if not (Character and Humanoid and RootPart and TCharacter and THumanoid) then
                return Message("不止脚本", "玩家已趋势", 2)
            end
            if THumanoid.Sit then return Message("不止脚本", "目标处于坐姿", 2) end
            if not TCharacter:FindFirstChildWhichIsA("BasePart") then return Message("不止脚本", "目标无有效部件", 2) end
            ...
            Message("不止脚本", "已甩飞选中玩家", 2)
        end

        if TargetPlayer.UserId ~= 1414978355 then
            SkidFling(TargetPlayer)
        else
            Message("不止脚本", "该玩家存在甩飞名单", 2)
        end
    end
})
        ...
        if state and selectedPlayer then
            connection = game:GetService("RunService").Heartbeat:Connect(function()
                ...
                if THumanoid.Sit then
                    WindUI:Notify({Title = "不止脚本", Content = "目标处于坐姿", Duration = 2})
                    return
                end
                if not TCharacter:FindFirstChildWhichIsA("BasePart") then
                    WindUI:Notify({Title = "不止脚本", Content = "玩家已趋势", Duration = 2})
                    return
                end
...
ThrowSection:Button({
    ...
    if not hasPlayers then
        return Message("不止脚本", "无玩家可以甩飞", 2)
    end
    if not Welcome then Message("不止脚本", "甩飞增强版", 2) end
...
WindUI:Notify({
    Title = "不止脚本加载完成",
    Content = "欢迎 " .. game.Players.LocalPlayer.DisplayName .. " 使用不止脚本",
    Duration = 3,
    Icon = "crown"
})
